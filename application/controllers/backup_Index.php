...
class Index extends MY_Controller {
        ...
        if($this->input->post()){
            ...
            if($this->form_validation->run()===FALSE)
            {
                ...
                if (count($mem_row) == '0')
                {
                    ...
                    $save_data = array('mem_fname' => ucfirst($post['fname']),'mem_lname' => ucfirst($post['lname']),'mem_email' => $post['email'],'mem_phone' => $post['phone'], 'mem_pswd' => doEncode($post['password']), 'mem_code' => $rando, 'mem_type' => 'student','mem_status'=>0,'mem_hear_about'=>$post['hear_about'],'mem_last_login' => date('Y-m-d h:i:s'),'mem_referral_code'=>$mem_referral_code);
                    ...
                    $res['redirect_url'] = site_url('tutor-signup');
                    $res['status'] = 1;
                    $res['frm_reset'] = 1;
                } else {
        ...
        if($this->input->post()){
            ...
            if($this->form_validation->run()===FALSE)
            {
                ...
                if (count($mem_row) == '0')
                {
                    ...
                    $arr = array('mem_fname' => ucfirst($post['fname']),'mem_lname' => ucfirst($post['lname']),'mem_email' => $post['email'],'mem_phone' => $post['phone'], 'mem_pswd' => doEncode($post['password']), 'mem_code' => $rando, 'mem_type' => 'tutor','mem_status'=>0,'mem_hear_about'=>$post['hear_about'],'mem_last_login' => date('Y-m-d h:i:s'),'mem_referral_code'=>$mem_referral_code);
    ...
    function tutor_multi_signup()
    {
        ...
        if($this->input->post()) {
            ...
            if($post['Math'] || $post['Science'] || $post['Language'] || $post['English'] || $post['Test'] || $post['Computer'] || $post['Business'] || $post['History'] || $post['Music'] || $post['Special_needs'] || $post['Sports'] || $post['Art'] || $post['Education']) {

                //following subjects
                $subjects = array('Math' => $post['Math'], 'Science' => $post['Science'], 'Language' => $post['Language'], 'English' => $post['English'], 'Test' => $post['Test'], 'Computer' => $post['Computer'], 'Business' => $post['Business'], 'History' => $post['History'], 'Music' => $post['Music'], 'Special_needs' => $post['Special_needs'], 'Sports' => $post['Sports'], 'Art' => $post['Art'], 'Education' => $post['Education']);
            ...
            if ($mem_row != '0')
            {
                ...
                $arr = array('mem_subjects' => $subjects, 'mem_education' => $Education, 'mem_type' => 'tutor','mem_status'=>0,'mem_hear_about'=>$post['hear_about'],'mem_last_login' => date('Y-m-d h:i:s'), 'mem_hourly_rate' => $post['hourly_rate'], 'mem_address1' => $post['address'], 'mem_travel_radius' => $post['travel_radius'], 'mem_cancel_policy' => $post['selectCancelPolicy'], 'mem_profile_heading' => $post['profile_headline'], 'mem_image' => $data['mem_image'], 'mem_bio' => $post['bio'], 'mem_street' => $post['street'], 'mem_city' => $post['city'], 'mem_zip' => $post['zip'], 'mem_country_id' => $get_row->id, 'mem_graduation_year' => $post['grad_year'], 'mem_major_subject' => $post['major'], 'mem_school_name' => $post['college']);
                ...
                /*It's for user*/
                    $msg_body = "Registered Successfully, you can login once we review your profile.";
                ...
                /* this is for admin */
                $link = "<a href='https://crainly.com/admin/tutors/manage/".$mem_row."''>Edit here</a>";
                $msg_body = "New user signup, waiting for approval. You can see profile for the user here click on this link: ".$link."";
                ...
                $res['msg'] = showMsg('success', 'Registered Successfully, you can login once we review your profile.');
                $this->session->set_flashdata('success', 'Your profile submit successfully, you can login once we review your profile.');
                $res['redirect_url'] = site_url('tutor-signup');
                $res['status'] = 1;
                $res['frm_reset'] = 1;
        ...
        if ($post=$this->input->post()) {
            ...
            if (count($rows) > 0) {
                ...
                foreach ($rows as $row) {
                    ...
                    <li>
                    <div class="inside"><span>'.round($row->total_time,1).' Hours</span>tutored</div>
                    </li>
                    </ul>
                    </div>
                    </li>';
